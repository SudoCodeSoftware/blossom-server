        ...
        if ($req_type == 'create'){
            ...
            if ($req_type ==  'delete'){
                ...
                if (trim($_POST["message"]) != "" ) {
                    $chat_query = "UPDATE active_conversations SET messages = CONCAT(messages, '".$user_id.chr(31).$_POST["message"].chr(31)."'), cache = CONCAT(cache, '".$user_id.chr(31).$_POST["message"].chr(31)."') WHERE conversation_id = '".$conversation_id."' ";
                ...
                $response = $conn->query($chat_query);
                $output = explode(chr(31), $response->fetch_assoc()["messages"]);
                $chat_query = "SELECT messages FROM active_conversations WHERE conversation_id = '".$conversation_id."'";
                ...
                foreach (range(1, count($conversation_id)) as $number) {
                    ...
                    if ($response["user_1"] != $user_id) {
                        $user_data = getNamefromID(true, $response["user_1"], $conn, true);
                        $lastMessageIndex = count(explode(chr(31),$response['messages']));
                        if ($user_data["username"] != null) {
                            array_push($names, array($user_data["username"], $user_data["fb_id"], $user_data["uni"], $user_data["cover"], array( trim(explode(chr(31),$response['messages'])[$lastMessageIndex - 3]), (explode(chr(31),$response['messages'])[$lastMessageIndex - 2]))));
                        }
                    }
                    else {
                        $user_data = getNamefromID(true, $response["user_2"], $conn, true);
                        $lastMessageIndex = count(explode(chr(31),$response['messages']));
                        if ($user_data["username"] != null) {
                            array_push($names, array($user_data["username"], $user_data["fb_id"], $user_data["uni"], $user_data["cover"], array( trim(explode(chr(31),$response['messages'])[$lastMessageIndex - 3]), (explode(chr(31),$response['messages'])[$lastMessageIndex - 2]))));
                        }
